...
\begin{lem} 
\ollabel{lem:val-in-termmodel}
Let $\Struct M(\Gamma^*)$ be the term model of \olref{defn:termmodel}. Then 
$\Value{t}{M(\Gamma^*)} = t$.
\end{lem}
\begin{proof}
The proof is by induction on $t$, where the base case, where $t$ is a constant
symbol follows directly from the definition of the term model. For the induction step
assume $t_1$, \dots,$t_n$ are closed terms such that $\Value{t_i}{M(\Gamma^*)} = t_i$ 
and that $f$ is an $n$-ary function symbol. Then 
\begin{align*}
\Value{f(t_1,\ldots,f_n)}{M(\Gamma^*)} &= \Assign{f}{M(\Gamma^*)}(\Value{t_1}{M(\Gamma^*)}, 
